...
function parseVariablesFromLabel(kvMap, label) {
    ...
    Object.keys(kv).forEach(function (key) {
        ...
        switch (_typeof(_options.defaultKeyValueMap[key])) {
            case "string":
            ...
            case "boolean":
                ...
                break;
            default:
                logger.error("include-codeblock: parseVariablesFromLabel: key type `" + _typeof(_options.defaultKeyValueMap[key]) + "` unknown (see options.js)");
                break;
        }
        // Val type cast to string.
        var regStr = beginEx + sepEx + spacesEx + keyEx + spacesEx + kvsepEx + spacesEx + valEx + spacesEx + sepEx + endEx;
        var reg = new RegExp(regStr);
        var res = label.match(reg);
        if (res) {
            kv[key] = (0, _options.convertValue)(res[2], _typeof(_options.defaultKeyValueMap[key]));
        }
    });
 ...
 * @return {string}
 */
function getContent(filePath, originalPath) {
...
function embedCode(kvMap, _ref2) {
    ...
    var code = getContent(filePath, originalPath);
